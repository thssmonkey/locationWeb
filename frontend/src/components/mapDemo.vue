...
<script>
  ...
  export default {
    ...
    created(){
      this.$root.Bus.$on('refreshEvent', value => {
        this.longitude = value.split(",")[0]
    ...
    methods: {
      ready () {
        // 网关总数量
        var gateways = this.GLOBAL.GATEWAY_NUM
        // 创建地图
        ...
        for (var i = 0; i < markerArr.length; i++) {
          ...
          var maker = addMarker(new window.BMap.Point(p0, p1), i, gateways)
        ...
        // 划线
        // 网关之间的连线
        for (var i = 0; markerArr.length > 0 && i < gateways; i++) {
          var polyline = new BMap.Polyline([new BMap.Point(lngs[i], lats[i]), new BMap.Point(lngs[(i + 1) % gateways], lats[(i + 1) % gateways])], {strokeColor:"red", strokeWeight:2, strokeOpacity:0.5})
          map.addOverlay(polyline)
          addLineInfo(polyline, markerArr[i], markerArr[(i + 1) % gateways])
        }
        // 节点与网关之间的连线
        for (var i = gateways; i < markerArr.length; i++) {
        ...
        // 添加标注
        function addMarker(point, index, num) {
          if (index < num) {
